...
async def markdown_dossier(identity_id: str, db: DbSession, user: CurrentUser) -> Response:
    ...
    filename = f"silentshift-dossier-{identity['username']}-{datetime.utcnow():%Y%m%d%H%M}.md"
    return Response(
        content=body,
...
async def json_dossier(identity_id: str, db: DbSession, user: CurrentUser) -> dict:
    ...
    return {
        "case_reference": f"SS-DOSSIER-{bundle['identity']['username'].upper()}-"
...
async def events_csv(identity_id: str, db: DbSession, user: CurrentUser) -> Response:
    ...
    filename = f"silentshift-events-{identity.username}-{datetime.utcnow():%Y%m%d}.csv"
    return Response(
        content="\n".join(lines),
        media_type="text/csv",
        headers={"Content-Disposition": f'attachment; filename="{filename}"'},
...
def _render_dossier(bundle: dict, chain: list[dict], author: str) -> str:
    ...
    return f"""# SilentShift Forensic Dossier

| | |
|---|---|
| **Case reference** | `SS-DOSSIER-{identity["username"].upper()}-{now:%Y%m%d%H%M}` |
| **Classification** | CONFIDENTIAL — Internal security use |
| **Generated** | {now:%Y-%m-%d %H:%M:%S} UTC |
| **Generated by** | {author} |
| **Platform** | SilentShift v{settings.VERSION} |
